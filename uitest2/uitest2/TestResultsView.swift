        ...
        ScrollView {
            VStack(spacing: 25) {
                ...
                VisionTestResultView(
                    rightEyeLogMAR: VisionTestManager.shared.rightEyeLogMAR,
                    leftEyeLogMAR: VisionTestManager.shared.leftEyeLogMAR
                )
                ...
                VStack(alignment: .leading, spacing: 15) {
                    ...
                    InfoRow(
                        title: "LogMAR Test",
                        description: "logmar_explanation".localized
                    )
...
struct ReportCardView: View {
    ...
    private var reportCard: some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack {
                    Text("\("right_eye".localized): LogMAR \(String(format: "%.2f", rightEyeLogMAR))")
                    Spacer()
                    StatusIndicator(value: rightEyeLogMAR, threshold: 0.3)
                }
                
                HStack {
                    Text("\("left_eye".localized): LogMAR \(String(format: "%.2f", leftEyeLogMAR))")
                    Spacer()
                    StatusIndicator(value: leftEyeLogMAR, threshold: 0.3)
                }
            }
            .padding()
    ...
    private func saveReportAsImage() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            let renderer = UIGraphicsImageRenderer(bounds: CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width - 40, height: 600))
...
struct StatusIndicator: View {
    ...
    private func circleColor(for index: Int) -> Color {
        ...
        let percentage = 1.0 - normalizedValue  // 反轉，使較小值代表更好的視力
